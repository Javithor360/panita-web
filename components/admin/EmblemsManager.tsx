...
function SortableEmblemItem({
  emblem,
  onEdit,
}: {
  emblem: Emblem & { edition: Edition | null };
  onEdit: (e: Emblem) => void;
}) {
  const {
    attributes,
    listeners,
...
export function EmblemsManager() {
  ...
  const [selectedEmblem, setSelectedEmblem] = useState<Partial<Emblem> | null>(
    null,
  );
  ...
  const [viewType, setViewType] = useState<"emblems" | "titles">("emblems");

  // Tab: Users
  const [emblemUsers, setEmblemUsers] = useState<
    {
      id: number;
      ign: string | null;
      discord_name: string;
  ...
  return (
    <>
            ...
            {!selectedEmblem ? (
              <div className="flex flex-col gap-4">
                {loading ? (
                  ...
                  <div className="flex flex-col gap-4">
                    <div className="flex gap-1.5 bg-card/60 backdrop-blur-sm rounded-xl border border-border p-1.5 w-full mx-auto shadow-sm">
                      <button
                        onClick={() => setViewType("emblems")}
                        className={`flex-1 flex items-center justify-center gap-2 py-2.5 px-4 rounded-lg font-bold text-sm transition-all duration-300 cursor-pointer ${viewType === "emblems" ? "" : "text-muted-foreground hover:bg-white/5 hover:text-white/90"}`}
                        style={
                          viewType === "emblems"
                            ? {
                                backgroundColor:
                      ...
                      <button
                        onClick={() => setViewType("titles")}
                        className={`flex-1 flex items-center justify-center gap-2 py-2.5 px-4 rounded-lg font-bold text-sm transition-all duration-300 cursor-pointer ${viewType === "titles" ? "" : "text-muted-foreground hover:bg-white/5 hover:text-white/90"}`}
                        style={
                          viewType === "titles"
                            ? {
                                backgroundColor:
                    ...
                    >
                      <SortableContext
                        items={emblems
                          .filter((e) =>
                            viewType === "emblems" ? e.icon_url : !e.icon_url,
                          )
                          .map((e) => e.id)}
                      ...
                      >
                        {emblems
                          ...
                          .map((e) => (
                            <SortableEmblemItem
                              key={e.id}
                              emblem={e as Emblem & { edition: Edition | null }}
                              onEdit={handleEdit}
                            />
                          ))}
                      </SortableContext>
                    ...
                    <button
                      ...
                      style={
                        {
                          backgroundColor:
                            "color-mix(in srgb, var(--profile-glow) var(--btn-opacity, 15%), transparent)",
                          color: "var(--profile-glow)",
                        } as React.CSSProperties
                      }
                    >
                      <Plus className="w-4 h-4" />
                      Crear Emblema o Título
                    </button>
                  </div>
                )}
              </div>
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-2">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="relative group">
                          ...
                          <span className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 w-56 p-2 text-xs bg-popover text-popover-foreground border rounded-md shadow-md opacity-0 pointer-events-none group-hover:opacity-100 transition-opacity z-50">
                            Si no se selecciona imagen, el emblema funcionará
                            como un galardón o título textual.
                          </span>
                        </span>
                      </div>
                    ...
                    <div className="flex flex-col gap-2 relative">
                      <label className="text-sm font-medium flex items-center gap-2">
                        Origen
                        {!iconUrl && !iconFile && (
                          <span className="text-[10px] text-destructive bg-destructive/10 px-1.5 py-0.5 rounded-sm uppercase tracking-wider font-bold">
                            Requerido para títulos
                          </span>
                        )}
                      </label>
                      <button
                        type="button"
                        onClick={() =>
                          setIsEditionSelectOpen(!isEditionSelectOpen)
                        }
                  ...
                  >
                      ...
                      <div className="flex flex-col gap-1 p-2 border rounded-md bg-secondary/10 max-h-48 overflow-y-auto">
                        {searching ? (
                          <p className="text-xs text-center py-2 text-muted-foreground">
                            ...
                            (u) => !emblemUsers.some((eu) => eu.id === u.id),
                          ).length > 0 ? (
                          searchResults
                            .filter(
                              (u) => !emblemUsers.some((eu) => eu.id === u.id),
                    ...
                    <div className="flex flex-col gap-2 mt-2">
                      <h4 className="text-sm font-semibold flex items-center justify-between">
                        ...
                        <span className="bg-foreground text-background px-2 py-0.5 rounded-full text-xs font-semibold mr-1">
                          {emblemUsers.length}
                        </span>
                      </h4>
                      ...
                      {emblemUsers.length === 0 ? (
                        ...
                        <div className="flex flex-col gap-2 pr-1">
                          {emblemUsers.map((u) => (
                            <div
                              key={u.id}
                              className="flex items-center justify-between p-2.5 border rounded-md bg-card"
                            >
                              <div className="flex items-center gap-3">
                                <div className="w-8 h-8 rounded-md bg-secondary overflow-hidden flex-shrink-0">
                                  {/* eslint-disable-next-line @next/next/no-img-element */}
      ...
      <AlertDialog
        open={!!emblemToDelete}
        onOpenChange={(open) => !open && setEmblemToDelete(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              el emblema y lo removerá de todos los usuarios que lo posean.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
