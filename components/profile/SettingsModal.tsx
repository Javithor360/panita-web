...
import { changePasswordAction, changeIgnAction } from "@/app/actions/auth";
...
export function SettingsModal({
  ...
  const [isIgnSectionOpen, setIsIgnSectionOpen] = useState(false);
  ...
  const [ignSuccess, setIgnSuccess] = useState("");
  const [isIgnPending, startIgnTransition] = useTransition();
  ...
  const handleIgnSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setIgnError("");
    setIgnSuccess("");

    if (!newIgn) {
      setIgnError("Por favor ingresa un nombre de usuario.");
      return;
    }

    if (newIgn === user?.ign) {
      setIgnError("El nombre de usuario es igual al actual.");
      return;
    }

    startIgnTransition(async () => {
      const formData = new FormData();
      formData.append("newIgn", newIgn);

      const result = await changeIgnAction(null, formData);

      if (result?.error) {
        setIgnError(result.error);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent
        ...
        className="w-[95vw] sm:max-w-[700px] md:max-w-[850px] max-h-[90vh] p-0 overflow-hidden bg-card border-border/50"
      >
        <div className="flex flex-col md:flex-row h-[85vh] md:h-[650px] max-h-[850px]">
          ...
          <div className="w-full md:w-1/3 bg-black/20 p-4 border-b md:border-b-0 md:border-r border-border/50 flex flex-col">
            <div className="flex items-center justify-between mb-2 md:mb-4">
              <h2 className="font-bold text-lg text-foreground/90 pl-2">
                Ajustes
              </h2>
              {/* Botón X Móvil */}
              <button
              ...
              <div className="space-y-6 relative">
                <div className="mb-6">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Administra la configuración de seguridad y detalles de tu
                    cuenta.
                  </p>
                </div>
                ...
                <div className="border border-border/50 rounded-lg overflow-hidden bg-background/30 mt-4">
                  <div
                    className="flex items-center gap-3 p-4 cursor-pointer select-none hover:bg-secondary/20 transition-colors"
                    onClick={() => setIsIgnSectionOpen(!isIgnSectionOpen)}
                  >
                    <ChevronDown
                      className={cn(
                        "w-5 h-5 transition-transform text-muted-foreground shrink-0",
                        !isIgnSectionOpen ? "-rotate-90" : "rotate-0",
                      )}
                    />
                    <div>
                  ...
                  >
                    <div className="overflow-hidden">
                      <form
                        onSubmit={handleIgnSubmit}
                        className="p-4 pt-0 space-y-4 border-t border-border/50 mt-2"
                      >
                        <p className="text-sm text-muted-foreground mt-4 mb-2">
                          Ten en cuenta que debe tener entre 3 y 16 caracteres.
                        </p>

                        {!user?.trusted_author && (
                          <div className="flex items-start gap-2 p-3 bg-secondary/30 text-muted-foreground rounded-md text-sm border border-border/50">
                            <Lock className="w-4 h-4 mt-0.5 shrink-0" />
                            <p>
                              Necesitas tener permisos de editor para poder
                        ...
                        <div className="space-y-2">
                          ...
                          <input
                            ...
                            disabled={!user?.trusted_author || isIgnPending}
                            className="w-full p-2 bg-background border rounded-md disabled:opacity-50 disabled:cursor-not-allowed"
                            placeholder="Ej. mi_nuevo_ign"
                          />
                        </div>
                        ...
                        <div className="pt-4 flex justify-end">
                          ...
                          >
                            <User className="w-4 h-4" />
                            {isIgnPending ? "Actualizando..." : "Actualizar"}
                          </Button>
                        </div>
                      </form>
                    </div>
                ...
                <div className="border border-border/50 rounded-lg overflow-hidden bg-background/30">
                  ...
                  >
                    <ChevronDown
                      className={cn(
                        "w-5 h-5 transition-transform text-muted-foreground shrink-0",
                        !isPasswordSectionOpen ? "-rotate-90" : "rotate-0",
                      )}
                    />
                    <div>
                  ...
                  >
                    <div className="overflow-hidden">
                      ...
                      >
                        <p className="text-sm text-muted-foreground mt-4 mb-2">
                          Asegúrate de usar al menos 5 caracteres y un símbolo
                          especial.
                        </p>
                        <div className="space-y-2">
                          <label
