...
type UserEmblem = UserProfilePayload["emblems"][number];

type FormWithResponses = Form & {
...
interface ProfileViewProps {
  ...
  joinedDate: string;
  glowColor: string;
  userRoles: UserProfilePayload["roles"];
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="relative -mt-24 sm:-mt-32 flex flex-col items-center">
            ...
            <div className="mt-6 flex flex-col sm:flex-row flex-wrap justify-center items-center gap-3 sm:gap-6">
              {userRoles.length > 0 ? (
                userRoles.map((role: UserRole, index: number) => {
                  const isGradient = role.color.includes("gradient");
                  return (
                    <Fragment key={role.id}>
                      <span
                        className={`font-minecraft font-bold text-xl md:text-2xl tracking-widest drop-shadow-sm select-none ${isGradient ? "bg-clip-text text-transparent" : ""}`}
                        style={{
                          backgroundImage: isGradient ? role.color : undefined,
                          color: isGradient ? undefined : role.color,
                          filter: isGradient
                            ? "drop-shadow(0 0 8px rgba(255,255,255,0.2))"
                            : `drop-shadow(0 0 8px ${role.color}60)`,
                        }}
                      >
            ...
            <div className="mt-12 w-full">
              ...
              {visualEmblems.length > 0 ? (
                <div className="flex flex-wrap justify-center gap-6">
                  {visualEmblems.map((emblem: UserEmblem) => (
                        ...
                        <div className="flex flex-col sm:flex-row items-center gap-6 py-4">
                          ...
                          <div className="flex flex-col gap-3 w-full">
                            <p className="text-muted-foreground text-sm leading-relaxed text-center sm:text-left">
                              {emblem.description ||
                                "Este emblema es símbolo de la participación en un evento de alto renombre, otorgado a los participantes más honorables."}
                            </p>
                          </div>
                        </div>
                          ...
                          {Math.max(0, emblem._count.users - 1) === 0 ? (
                            <p
                              ...
                              style={{
                                ...
                                filter:
                                  "drop-shadow(0 0 8px var(--profile-glow))",
                              }}
                            >
                            ...
                            <p className="text-xs text-muted-foreground/60 italic text-center">
                              Otros {Math.max(0, emblem._count.users - 1)}{" "}
                              usuarios también tienen este emblema.
                            </p>
                          )}
                        </div>
                      </DialogContent>
                ...
                >
                  ...
                  <p className="text-sm font-medium opacity-80">
                    Aún no ha desbloqueado ningún emblema.
                  </p>
                </div>
              )}
            </div>
            ...
            {/* Profile Forms (Only if it's personal profile) */}
            {showForms && forms.length > 0 && <ProfileForms forms={forms} />}

            {/* Admin Panel (Only if and admin is watching their profile) */}
            {showAdminPanel && <AdminPanel glowColor="var(--profile-glow)" />}
          </div>
        </div>
      </div>
