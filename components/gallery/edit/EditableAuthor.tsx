      ...
      >
        {isPending ? (
          ...
          <Image unoptimized
            ...
            width={16}
            height={16}
            className="size-4 rounded-sm bg-black/20 shrink-0"
          />
        ) : (
      ...
      {open && (
        <div className="absolute top-full left-0 mt-2 w-[300px] z-50 rounded-md border border-white/10 bg-black/95 backdrop-blur-md text-white shadow-xl flex flex-col overflow-hidden">
          ...
          <div className="max-h-[300px] overflow-y-auto p-2 flex flex-col gap-1">
            {isSearching && (
              <div className="p-3 text-center text-sm text-white/50">
                Buscando...
              </div>
            )}
            ...
            {results.map((user) => (
              <button
                key={user.id}
                onClick={() =>
                  handleSelect(user.id, user.ign || user.discord_name, user.ign)
                }
                className="flex items-center gap-3 p-2 rounded-md hover:bg-white/10 text-left transition-colors cursor-pointer"
              >
                <Image unoptimized
