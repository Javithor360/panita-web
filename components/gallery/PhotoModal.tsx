...
export function PhotoModal({
  ...
  /** True when the current entry is a video (not a photo). */
  const isVideo = localPhoto.media_type === "video";
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Arrow key navigation — only for images (video is handled by YouTube player)
      if (
        e.key === "ArrowRight" &&
        onNext &&
        scale === 1 &&
  ...
  }, [onClose, onNext, onPrev, scale, isEditMode, isVideo]);
  ...
  const handleDownload = async () => {
    // For videos, open the YouTube watch page instead of downloading.
    if (isVideo) {
      window.open(
        `https://www.youtube.com/watch?v=${localPhoto.youtube_id}`,
        "_blank",
      );
      return;
    }
  ...
  const handleWheel = (e: React.WheelEvent) => {
    if (showDetails || isVideo) return;
    if (e.deltaY < 0) handleZoom(0.25);
    else handleZoom(-0.25);
  };

  const handleMouseDown = (e: React.MouseEvent) => {
    if (scale <= 1 || showDetails || isVideo) return;
    setIsDragging(true);
    setDragStart({
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    if (!isDragging || scale <= 1 || showDetails || isVideo) return;
    setPosition({
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    if (scale > 1 || !touchStart || isVideo) return;
  ...
  const resolvedTags = localPhoto.tagIds
  ...
  if (localPhoto.date_taken) {
    const dateStr =
      localPhoto.date_taken instanceof Date
        ? localPhoto.date_taken.toISOString()
        : String(localPhoto.date_taken);
  ...
  const hasDescription = Boolean(
    localPhoto.description && localPhoto.description.trim() !== "",
  );
  ...
  if (
    ...
    optimizedUrl = optimizedUrl.replace(
      "/upload/",
      "/upload/c_limit,w_1920,q_auto,f_auto/",
    );
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex flex-col justify-start bg-black/40 backdrop-blur-lg lg:backdrop-blur-xs animate-in fade-in zoom-in-95 duration-500 ease-out">
      {/* Top Header */}
      <div className="absolute top-0 left-0 right-0 px-6 py-6 md:px-12 md:py-8 flex flex-col-reverse lg:flex-row justify-between lg:items-start gap-4 lg:gap-0 z-10 pointer-events-none transition-all duration-500 ease-in-out opacity-100 bg-gradient-to-b from-black/95 via-black/50 to-transparent min-h-[150px]">
        {/* Left side info */}
        <div className="flex flex-col gap-1.5 pointer-events-auto mt-1 w-full lg:w-auto">
          ...
          <div className="flex flex-wrap items-center gap-3 text-white/90 font-medium">
            ...
            {(localPhoto.edition_name || isEditMode) && (
          ...
          {/* Row 2: Date and visibility toggle */}
          <div className="flex flex-wrap items-center gap-3 text-white/60 text-sm">
            {isEditMode ? (
              <EditableDate
                value={
                  localPhoto.date_taken
                    ? new Date(localPhoto.date_taken)
                        .toISOString()
                        .split("T")[0]
                    : ""
                }
            ...
            {isEditMode && (
              <div className="flex items-center gap-2 w-full min-[480px]:w-auto">
                <span className="hidden min-[480px]:inline text-white/40">
                  •
                </span>
                ...
                <button
                  ...
                  onClick={() => {
                    startTransition(async () => {
                      await updatePhoto(localPhoto.id, {
                        enabled: !localPhoto.enabled,
                      });
                      const newPhoto = {
                        ...localPhoto,
                        enabled: !localPhoto.enabled,
                ...
                >
                  <span
                    className={`pointer-events-none inline-block size-3.5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${localPhoto.enabled ? "translate-x-2" : "-translate-x-2"}`}
                  />
                </button>
              </div>
            )}
      ...
      >
            ...
            style={{
              ...
              filter:
                !localPhoto.enabled && canEdit
                  ? "grayscale(100%) opacity(50%) blur(8px)"
                  : "blur(8px)",
            }}
          />
        )}

        {/* YouTube embed — for video entries */}
        {isVideo && localPhoto.youtube_id && (
          <div
            className="w-full max-w-5xl aspect-video rounded-xl overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-300"
            onClick={(e) => e.stopPropagation()}
          >
            <iframe
              src={`https://www.youtube.com/embed/${localPhoto.youtube_id}?rel=0&modestbranding=1`}
        ...
        {!localPhoto.enabled && canEdit && (
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-black/60 backdrop-blur-md px-6 py-3 rounded-full text-white font-medium text-lg pointer-events-none flex items-center gap-2">
            ...
            <span>{isVideo ? "Video Oculto" : "Foto Oculta"}</span>
          </div>
        )}
        ...
        <div className="flex flex-col gap-3 w-full mt-auto animate-in fade-in duration-300">
          ...
          <div className="flex items-center gap-3 md:gap-4 pointer-events-auto shrink-0">
            {isEditMode ? (
              ...
              <h2
                className={`font-bold text-white leading-tight drop-shadow-md transition-all duration-500 ${showDetails ? "text-xl md:text-5xl mb-4" : "text-xl md:text-3xl"}`}
              >
                {localPhoto.title}
              </h2>
            )}
            ...
            {(hasDescription || isEditMode) && (
              ...
              >
                <svg
                  className={`size-3.5 md:size-5 transition-transform duration-300 ${showDetails ? "rotate-180" : ""}`}
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
