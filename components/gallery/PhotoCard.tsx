...
interface PhotoCardProps {
  ...
  imageUrl: string;
  priority?: boolean;
  /** Whether the card represents an image or a video. Defaults to 'image'. */
...
export function PhotoCard({
  ...
  return (
    ...
    >
      <div className="relative w-full aspect-video overflow-hidden rounded-t-[0.3rem] bg-black">
        ...
        {(!imageLoaded || hasError) && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10 bg-black">
            ...
            />
            {mediaType === "video" && (
              <div className="absolute inset-x-0 bottom-4 flex flex-col gap-2 items-center justify-end opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-2 group-hover:translate-y-0 z-50">
                <span className="text-white/90 font-medium text-xs bg-black/60 px-3 py-1.5 rounded-full backdrop-blur-md border border-white/10 shadow-xl flex items-center gap-2">
                  <Loader2 className="size-3.5 text-white/70 animate-spin" />
                  Procesando miniatura...
                </span>
              </div>
            )}
          </div>
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100 pointer-events-none z-30" />
        ...
        {/* Video badge */}
        {mediaType === "video" && (
          <span className="absolute top-2 right-2 bg-black/70 backdrop-blur-sm text-white text-[10px] font-bold px-2 py-0.5 rounded-full border border-white/20 tracking-wider uppercase z-40">
            Video
          </span>
        )}
      </div>
      <CardFooter className="flex flex-col flex-1 items-start p-4 gap-3 bg-card/50 backdrop-blur-sm border-t border-muted/20">
        <h3 className="font-semibold text-sm line-clamp-1 group-hover:text-primary transition-colors shrink-0">
          {title}
        </h3>
