...
export function FormRenderer({ form, hasResponded, isLoggedIn, user }: { form: any, hasResponded: boolean, isLoggedIn: boolean, user?: any }) {
    ...
    return (
      <Card className="p-8 bg-card border-border shadow-lg rounded-xl text-center flex flex-col items-center gap-4">
        ...
        <p className="text-muted-foreground text-sm max-w-md">
          Para participar en el formulario <strong>{form.title}</strong>, necesitas iniciar sesión con tu cuenta de Discord.
        </p>
        <Link 
    ...
    return (
      <Card className="p-8 bg-card border-t-8 border-t-emerald-500 border-x-border border-b-border shadow-lg rounded-xl text-center flex flex-col items-center gap-4">
        ...
        <p className="text-muted-foreground">
          Tus respuestas para <strong>{form.title}</strong> han sido registradas exitosamente.
        </p>
        <Link 
          href="/profile"
  ...
  const handleNext = () => {
    ...
    if (missing.length > 0) {
      setError(`Faltan ${missing.length} preguntas obligatorias por responder.`);
      return;
    }
  ...
  const toggleCheckbox = (questionId: string, option: string) => {
    ...
    if (currentList.includes(option)) {
      updateAnswer(questionId, currentList.filter((o: string) => o !== option));
    } else {
    ...
    <div className="flex flex-col gap-6">
        ...
        <div className="mt-4 pt-4 border-t border-border flex flex-col sm:flex-row sm:items-start justify-between gap-5 text-sm font-medium">
          <div className="flex flex-col gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <span className="text-foreground font-semibold text-base">{ign}</span>
            </div>
          </div>
      ...
      <div className="flex flex-col gap-6 relative">
          ...
          <div className="rounded-xl shadow-md flex flex-col">
            ...
            {currentSection.description && (
              <div className="px-6 py-4 bg-card border-x border-b border-border rounded-b-xl">
                <p className="text-[15px] text-foreground whitespace-pre-wrap">{currentSection.description}</p>
              </div>
            )}
          </div>
        )}
        ...
        {currentSection.questions.map((q: any, i: number) => {
          const val = answers[q.id] || "";
          const isRequiredAndEmpty = q.is_required && (!val || (Array.isArray(val) && val.length === 0));
          const hasError = error && isRequiredAndEmpty;

          return (
            <Card key={q.id} style={{ zIndex: 50 - i }} className={`relative overflow-visible p-6 bg-card border ${hasError ? 'border-destructive ring-1 ring-destructive' : 'border-border'} rounded-xl shadow-sm transition-all`}>
              <div className="mb-2">
                ...
                {q.type === 'SINGLE_CHOICE' && (
                  <div className="flex flex-col gap-1.5">
                    {(JSON.parse(q.options || "[]")).map((opt: string, i: number) => (
                      <label key={i} className="flex items-center gap-3 cursor-pointer group p-2 -ml-2 rounded-lg hover:bg-secondary/40 transition-colors">
                        <div 
                          className="w-5 h-5 rounded-full border-[1.5px] flex items-center justify-center transition-colors shadow-sm"
                          style={{ borderColor: val === opt ? 'var(--profile-glow, var(--primary))' : 'var(--muted-foreground)' }}
                        >
                          {val === opt && <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: 'var(--profile-glow, var(--primary))' }} />}
                        </div>
                        <input 
                          type="radio" 
                          name={`q-${q.id}`} 
                      ...
                      return (
                        <label key={i} className="flex items-center gap-3 cursor-pointer group p-2 -ml-2 rounded-lg hover:bg-secondary/40 transition-colors">
                          <div 
                            ...
                            style={{ 
                              backgroundColor: isChecked ? 'var(--profile-glow, var(--primary))' : 'transparent',
                              borderColor: isChecked ? 'var(--profile-glow, var(--primary))' : 'var(--muted-foreground)'
                            }}
                          >
                            {isChecked && (
                              <svg className="w-3.5 h-3.5 drop-shadow-sm" style={{ color: 'var(--profile-glow-text, #ffffff)' }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
            ...
            <div className="flex items-center gap-3">
              <div className="w-full sm:max-w-[250px] h-2.5 bg-secondary/80 rounded-full overflow-hidden shadow-inner">
                <div 
                  ...
                  style={{ width: `${progressPercent}%`, backgroundColor: 'var(--profile-glow, var(--primary))' }}
                />
              </div>
        ...
        <div className="flex items-center gap-3 self-end sm:self-auto shrink-0">
          ...
          {isLastSection ? (
            <button 
              ...
              style={{ backgroundColor: 'var(--profile-glow, var(--primary))', color: 'var(--profile-glow-text, #ffffff)' }}
            >
...
function SearchableDropdown({ options, value, onChange }: { options: string[], value: string, onChange: (val: string) => void }) {
  ...
  return (
    <div className="relative w-full md:w-1/2" ref={popoverRef}>
      ...
      >
        <span className={value ? "text-foreground" : "text-muted-foreground"}>{value || "Elegir opción..."}</span>
